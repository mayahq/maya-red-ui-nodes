      ...
      if (typeof source[element] !== "object") {
        ...
        if (element === "columns" && Array.isArray(source[element])) {
          source[element].forEach((sourceElement) => {
            let index = target[element].findIndex(
              (targetElement) =>
                (targetElement.field &&
                  sourceElement.field &&
                  targetElement.field === sourceElement.field) || // normal column object
                (targetElement.title &&
                  sourceElement.title &&
                  targetElement.title === sourceElement.title) // parent object with nested columns
            )
            if (index < 0) {
              // add new column
        ...
        var html = HTML(config, ui.isDark())
          ...
          beforeEmit: function (msg, value) {
            ...
            if (msg.hasOwnProperty("ui_control")) {
              ...
              if (
                ...
                Array.isArray(msg.ui_control.tabulator.columns) &&
          ...
          initController: function ($scope, events) {
            ...
            var createTable = function (
              ...
              // append keys in Array Object format where _identifier is getting visible = false
              const ColumnValue = []

              // push object inside columnvalue
              ColumnValue.push({
              ...
              if (keys == "Name") {
                tabledata.hideColumn = "Name"
              }

              console.log("columndata", columndata)
              // array map object keys coming from tabledata
              ...
              //     opts.height =
              //       tabledata.length > 0
              //         ? tabledata.length * y + 26
              //         : $scope.height * (sizes.sy + sizes.cy)
              //   } else {
              ...
              if (outputs > 0 && !opts.hasOwnProperty("cellClick")) {
                // default cellClick if not already defined by ui_control
                opts.cellClick = function (e, cell) {
                  $scope.send({
                    topic: cel.getField(),
                    payload: cell.getData(),
            ...
            $scope.$watch("msg", function (msg) {
              ...
              if (
                ...
                return msg // to avoid loopback from callbacks. No better solution jet. Help needed.
              ...
              if (msg && msg.hasOwnProperty("ui_control")) {
                var addValueOrFunction = function (config, param, value) {
                  ...
                  if (
                    typeof value === "string" &&
                    (valueFunction = value.parseFunction())
                  ) {
                ...
                var addObject = function (destinationObject, sourceObject) {
                  for (var element in sourceObject) {
                    if (!destinationObject[element])
                      destinationObject[element] = Array.isArray(
              ...
              if (msg && msg.hasOwnProperty("payload")) {
                ...
                if (
                  ...
                  if (
                    ...
                    if (
                      !msg.payload.hasOwnProperty("arguments") ||
                      !Array.isArray(msg.payload.arguments)
                    ) {
                      msg.payload.arguments = []
                    }
                    if (msg.payload.returnPromise) {
                      $scope.table[msg.payload.command]
                        .apply($scope.table, msg.payload.arguments)
                        .then(function (...args) {
                          $scope.send({
                            topic: "success",
                            ui_control: {
                              callback: $scope.msg.payload.command,
                            },
                            return: $scope.msg.payload,
                          })
                        })
                      ...
                      $scope.table[msg.payload.command].apply(
                        $scope.table,
                        msg.payload.arguments
                      )
                    }
